...
function egalapprox(a, b) {
    let rep;
    if (a == b) { rep = '='; } else { rep = '\\approx'; }
    return rep;
}

function textegalapprox(a, b) {
    let rep;
    if (a == b) { rep = ''; } else { rep = 'environ'; }
    return rep;
}
...
function genererExercice() {
    ...
    let egapprox = egalapprox(cm3, prix2 / (prix1 * cm1 * cm2));
    let egalenviron = textegalapprox(cm3, prix2 / (prix1 * cm1 * cm2));

    enonce.innerHTML = `Un article coûte initialement ${prix1} €.<br>
Il subit une ${hb1} de ${taux1} % puis une ${hb2} de ${taux2} % et enfin une dernière évolution.<br>
Son prix final est de ${prix2v} €<br>
À quoi correspond l'évolution manquante ?` ;
    solution.innerHTML = `L'évolution manquante correspond ${egalenviron} à une ${hb3} de ${taux3} %`;
    correctiond.innerHTML = `On a : $${prix1}\\times \\left( 1 ${s1} \\dfrac{${taux1}}{100} \\right) \\times \\left( 1 ${s2} \\dfrac{${taux2}}{100} \\right) \\times \\text{CM}=${prix2v}$ donc <br>
$${prix1}\\times${cm1v}\\times${cm2v}\\times\\text{CM}=${prix2v}$ et donc <br>
$\\text{CM}=\\dfrac{${prix2v}}{${prix1}\\times${cm1v}\\times${cm2v}} ${egapprox} ${cm3v}$ ce qui correspond à une ${hb3} de ${taux3} % <br>(car $\\left( ${cm3v}-1 \\right)\\times 100 = ${s3}~${taux3}$)`;
}
