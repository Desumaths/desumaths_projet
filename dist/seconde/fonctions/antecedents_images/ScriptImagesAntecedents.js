...
let a1 ,b1 ,oppb1 ,sb1 ,soppb1 ,im1 ,pim1 ,a1im1 ,solim1 ,ante1 ,im2 ,pim2 ,a1im2 ,solim2 ,ante2 ,ante1b1 ,ante1frac ,ante1fracsimp ,sante1fracsimp ,solante1simp ,ante2b1 ,ante2frac ,
ante2fracsimp ,sante2fracsimp ,solante2simp ,a11 ,sa11 ,oppa11 ,soppa11 ,x1 ,px1 ,x1carre ,fx1 ,x11 ,b11 ,x2 ,px2 ,x2carre ,fx2 ,x12 ,b12 ,b11oppa11 ,b12oppa11 ,valint ,
oppvalint ,soppvalint ,af ,oppaf ,soppaf ,bf ,sbf ,x1f ,px1f ,x2f ,px2f ,afx1f ,x1foppvalint ,afx1fbf ,x1frac ,x1fracsimp ,sx1fracsimp ,simply1 ,afx2f ,
x2foppvalint ,afx2fbf ,x2frac ,x2fracsimp ,sx2fracsimp ,simply2 ,num ,denom ,frac1 ,frac1s ,frac1s0valint ,sfrac1s0valint ,frac1s1af ,frac1sbf ,sfrac1sbf ,oppfrac1s1af ,
soppfrac1s1af ,oppfrac1s0valint ,soppfrac1s0valint ,somx ,somreeel ,solfrac1 ,solfrac1s ,ssolfrac1s ,simply3 ,num2 ,num2valint ,snum2valint ,oppnum2valint ,soppnum2valint ,
somx2 ,somreeel2 ,solnum2 ,solnum2s ,ssolnum2s ,simply4 ,k;

function genererExercice() {

    a1 = nbreouoppose(nbrealéa(2, 15));
    b1 = nbrealéa(-10, 10);
    oppb1 = -b1;
    sb1 = rienplus(b1);
    soppb1 = rienplus(oppb1);
    im1 = nbrealéa(-10, 10);
    pim1 = parenthesegd(im1);
    a1im1 = a1 * im1;
    solim1 = a1 * im1 + b1;
    ante1 = nbrealéa(-10, 10);
    im2 = nbresdiff(im1);
    pim2 = parenthesegd(im2);
    a1im2 = a1 * im2;
    solim2 = a1 * im2 + b1;
    ante2 = nbresdiff(ante1);
    ante1b1 = ante1 - b1;
    ante1frac = simplifierfraction(ante1b1, a1);
    ante1fracsimp = fracsimp(ante1frac[0], ante1frac[1]);
    sante1fracsimp = rienmoins(ante1b1 / a1);
    solante1simp = solsimplifiee(ante1frac[0], ante1frac[1]);
    ante2b1 = ante2 - b1;
    ante2frac = simplifierfraction(ante2b1, a1);
    ante2fracsimp = fracsimp(ante2frac[0], ante2frac[1]);
    sante2fracsimp = rienmoins(ante2b1 / a1);
    solante2simp = solsimplifiee(ante2frac[0], ante2frac[1]);

    a11 = nbreouoppose(nbrealéa(1, 15));
    sa11 = rienplus(a11);
    oppa11 = -a11;
    soppa11 = rienplus(oppa11);
    x1 = nbrealéa(-15, 15);
    px1 = parenthesegd(x1);
    x1carre = x1 ** 2;
    fx1 = x1carre + a11;
    ...
    valint = nbreouoppose(nbrealéa(1, 5));
    oppvalint = -valint;
    soppvalint = rienplus(oppvalint);
    af = nbreouoppose(2, 10);
    oppaf = -af;
    soppaf = rienplus(oppaf);
    bf = nbrealéa(-10, 10);
    sbf = rienplus(bf);
    x1f = diffvi(valint);
    px1f = parenthesegd(x1f);
    x2f = diffvix1f(x1f, valint);
    px2f = parenthesegd(x2f);
    afx1f = af * x1f;
    x1foppvalint = x1f - valint;
    afx1fbf = afx1f + bf;
    x1frac = simplifierfraction(afx1fbf, x1foppvalint);
    x1fracsimp = fracsimp(x1frac[0], x1frac[1]);
    sx1fracsimp = rienmoins(afx1fbf / x1foppvalint);
    simply1 = simply(x1frac[0], x1frac[1]);
    afx2f = af * x2f;
    x2foppvalint = x2f - valint;
    afx2fbf = afx2f + bf;
    x2frac = simplifierfraction(afx2fbf, x2foppvalint);
    x2fracsimp = fracsimp(x2frac[0], x2frac[1]);
    sx2fracsimp = rienmoins(afx2fbf / x2foppvalint);
    simply2 = simply(x2frac[0], x2frac[1]);
    num = nbreouoppose(nbrealéa(1, 10));
    denom = denominateur(af, num);
    frac1 = simplifierfraction(num, denom);
    frac1s = fracsimp(frac1[0], frac1[1]);
    frac1s0valint = frac1s[0] * oppvalint;
    sfrac1s0valint = rienplus(frac1s0valint);
    frac1s1af = frac1s[1] * af;
    frac1sbf = frac1s[1] * bf;
    sfrac1sbf = rienplus(frac1sbf);
    oppfrac1s1af = -frac1s1af;
    soppfrac1s1af = rienplus(oppfrac1s1af);
    oppfrac1s0valint = -frac1s0valint;
    soppfrac1s0valint = rienplus(oppfrac1s0valint);
    somx = frac1s[0] + oppfrac1s1af;
    somreeel = frac1sbf + oppfrac1s0valint;
    solfrac1 = simplifierfraction(somreeel, somx);
    solfrac1s = fracsimp(solfrac1[0], solfrac1[1]);
    ssolfrac1s = rienmoins(somreeel / somx);
    simply3 = simply(solfrac1[0], solfrac1[1]);
    num2 = ndiffaf(af);
    num2valint = num2 * oppvalint;
    snum2valint = rienplus(num2valint);
    oppnum2valint = -num2valint;
    soppnum2valint = rienplus(oppnum2valint);
    somx2 = num2 + oppaf;
    somreeel2 = bf + oppnum2valint;
    solnum2 = simplifierfraction(somreeel2, somx2);
    solnum2s = fracsimp(solnum2[0], solnum2[1]);
    ssolnum2s = rienmoins(somreeel2 / somx2);
    simply4 = simply(solnum2[0], solnum2[1]);
